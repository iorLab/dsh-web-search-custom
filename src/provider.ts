/**
 * `DiySearchProvider`: a `WebSearchProvider` backed by the bbg gateway's
 * OpenAI-compatible Responses API. One `POST /responses` request carries the
 ...
 * @module @jay/dsh-web-search-diy/provider
 */
...
import type { BbgErrorResponse, BbgResponsesResponse } from './types.js'

/** Stable id this provider registers under. */
export const BBG_PROVIDER_ID = 'diy-responses'

/** Default gateway endpoint base; `/responses` is appended. */
export const BBG_DEFAULT_BASE_URL = 'https://your-gateway.example.com/v1'

/** Default gateway model name for the auxiliary search request. */
export const BBG_DEFAULT_MODEL = 'deepseek-v4-flash'

/** Default `search_context_size` sent with the `web_search` tool. */
export const BBG_DEFAULT_SEARCH_CONTEXT_SIZE = 'low'

/** Default upper bound on generated tokens for the Responses request. */
export const BBG_DEFAULT_MAX_OUTPUT_TOKENS = 1024

/** Attribution header sent on every request. */
const USER_AGENT = 'dsh-web-search-diy/0.2.0'

/** Resolved provider options for one search operation. */
export interface DiySearchProviderOptions {
 ...
 */
export function mapDiyResponse(response: BbgResponsesResponse): WebSearchResult {
...
/** The bbg-gateway-backed search provider; HTTP redirects fail as `WEB_PROVIDER_ERROR`. */
export class DiySearchProvider implements WebSearchProvider {
  readonly id = BBG_PROVIDER_ID
   ...
   */
  constructor(private readonly resolveOptions: () => DiySearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    try {
      ...
      throw new WebError(`bbg search request failed: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }

    if (!response.ok) {
      const status = response.status
      let message = `bbg gateway error (HTTP ${status})`
      try {
        const parsed = await response.json() as BbgErrorResponse
    ...
    try {
      const payload = await response.json() as BbgResponsesResponse
      return mapDiyResponse(payload)
    } catch (error: unknown) {
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      if (error instanceof WebError) throw error
      throw new WebError(`bbg returned an unprocessable response body: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }
  }
   ...
   */
  private async apiKey(options: DiySearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    try {
      ...
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      throw new WebError(`bbg search credential resolution failed: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }
    if (resolved !== undefined && resolved.length > 0) return resolved
    const ref = options.apiKeyEnv ?? 'BBG_AI_MIX_API_KEY'
    throw new WebError(
      `bbg search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-diy config',
...
function searchAborted(signal?: AbortSignal, fallback?: unknown): WebError {
  return new WebError('bbg search aborted', 'WEB_ABORTED', {
